...
resolve_latest_dir() {
    ...
    # Only consider dirs that contain a task plan — skips system dirs like sessions/.
    latest=""
    latest_mtime=0
    for entry in "${PLAN_ROOT}"/*/; do
        ...
        # Skip dirs that are not plan dirs. Support project-local zh plans.
        [ -f "${clean}/task_plan.zh.md" ] || [ -f "${clean}/task_plan.md" ] || continue
